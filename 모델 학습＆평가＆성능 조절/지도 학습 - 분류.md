...
# 테스트 데이터 → 예측
# pred = (분류)변수.predict(xtest)         # 이산 예측값
pred = (분류)변수.predict(xtest_scaled)    # 이산 예측값(스케일링)

# 평가
confusion_matrix(ytest, pred)   # 오차 행렬
성능지표(ytest, pred)            # 성능 지표
roc_auc_score(ytest, pred)      # AUC
